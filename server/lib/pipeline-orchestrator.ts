...
export async function runUnifiedPipeline(
    ...
    try {
        ...
        if (consensus.final_vote === 'fail') {
            ...
            const failedPrimitives = Object.entries(consensus.primitive_scores)
                .filter(([_, score]) => score < 0.70)
                .map(([primitive]) => primitive);

            const mutations = [];
            for (const primitive of failedPrimitives) {
                ...
                mutations.push({
                    ...
                    reason: `${consensus.primary_complaint} - Meta-learning recommended ${delta.toFixed(3)}`,
